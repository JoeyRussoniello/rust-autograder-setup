...
fn write_script(root: &Path, script_name: &str, contents: &str) -> Result<()> {
  ...
  create_dir_all(script_path.parent().unwrap()).with_context(|| format!("Failed to create directory for {}", script_path.to_string_lossy()))?;
  write(&script_path, contents).with_context(|| format!("Failed to write to {}", script_path.to_string_lossy()))?;
  Ok(())
}
...
pub fn write_commit_count_shell(root: &Path) -> Result<()> {
  write_script(root, SCRIPT_NAMES.commit_count, COMMIT_COUNT_SCRIPT_CONTENTS)?;
  Ok(())
}
...
pub fn write_branch_count_shell(root: &Path) -> Result<()> {
  write_script(root, SCRIPT_NAMES.branch_count, BRANCH_COUNT_SCRIPT_CONTENTS)?;
  Ok(())
}
